...
def search_recoll(phrase:str, params:dict):
    ...
    if sdir != '':
        ...
        mail_dir = os.getenv('MC_RECOLL_MAIL_DIR','/dummy')
        www_dir = os.getenv('MC_RECOLL_WWW_DIR','/dummy')

        mail_dir = mail_dir.replace('"','\"')
        www_dir = www_dir.replace('"','\"')

        q.execute(f'{phrase} -dir:"{mail_dir}" AND -dir:"{www_dir}" AND -rclcat:www AND issub:0')
        process_result_recoll(q, '', limit)

        q.execute(f'{phrase} -dir:"{mail_dir}" AND -dir:"{www_dir}" AND -rclcat:www AND -issub:0')
        process_result_recoll(q, '', limit)

        q.execute(f'{phrase} dir:"{mail_dir} AND -rclcat:www"')
