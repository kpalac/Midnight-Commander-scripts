...
my $temp;
my @glob1 = glob("/usr/share/applications/*.desktop");
my @glob2 = glob("/snap/*/current/meta/gui/*.desktop");


foreach my $desktop_file (@glob1, @glob2) {

    if (not -f $desktop_file) { next }
    ...
    print $name."|".$comment."|app://".$desktop_file."|".$mimes."|".$display."|".$categories."|".$keywords."\n"
